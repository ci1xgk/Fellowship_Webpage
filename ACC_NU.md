...
* The 2D model outputs are generated in two forms: 
   1. Multiresolution grid as `.vtk` file format (see section [*"Non-uniform grid output files (-xxxx.vtk)"*](/vtk.md) for visualising `.vtk` files).
   2. Standard uniform grid raster files (Recall section [*"Water depth or free-surface elevation output files (-xxxx.wd, -xxxx.elev)"*](/Merewether3-2.md)). 
...
The technical background of how the ACC solver is adapted on the non-uniform grid generated by the multiwavets Galerkin projection of the DEM, and parallised on the GPU can be found in [Sharifian et al. (2023)](https://gmd.copernicus.org/articles/16/2391/2023/gmd-16-2391-2023.html), which provides a demonstration of the solver performance for five real-world fluvial/pluvial case studies.
