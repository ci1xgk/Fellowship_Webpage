...
The latest version of LISFLOOD-FP includes a new solver option, known as the non-uniform ACC solver. This solver is achieved by applying the ACC solver on a static optimised non-uniform grid.  The non-uniform grid is generated by the application of the multiresolution analyses (MRA) of multiwavelets (MWs) on the topography data defined through a DEM.
