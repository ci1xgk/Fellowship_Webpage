...
Since time-varying inflow is used, the respective hydrograph must be provided in a `.bdy` file.  Figure below shows a snapshot of `ea5.bdy`, which provides the time series of unit-width discharge (q = Q/B = 3000/200 = 15 square meter per second). Note that since 20 cells are specified as point source, the opening width, B, is 200 m. 
